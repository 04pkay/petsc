  ...
  for (PetscInt istep = 1; istep < nrows; istep *= 2) {
    ...
    */
    for (istart = 0; istart < nrows; istart += 2 * istep) {
      ...
      i1    = istart;
      i1end = i1 + istep;
      if (i1end > nrows) i1end = nrows;
      i2    = istart + istep;
      i2end = i2 + istep;
      if (i2end > nrows) i2end = nrows;
  ...
  for (PetscInt istep = 1; istep < nnz; istep *= 2) {
    ...
    */
    for (istart = 0; istart < nnz; istart += 2 * istep) {
      ...
      i1    = istart;
      i1end = i1 + istep;
      if (i1end > nnz) i1end = nnz;
      i2    = istart + istep;
      i2end = i2 + istep;
      if (i2end > nnz) i2end = nnz;
...
PetscErrorCode spbas_power(spbas_matrix in_matrix, PetscInt power, spbas_matrix *result)
{
  ...
  PetscCheck(!in_matrix.values, PETSC_COMM_SELF, PETSC_ERR_ARG_INCOMP, "Input array must be sparseness pattern (no values)");
