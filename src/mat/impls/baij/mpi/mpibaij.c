...
static PetscErrorCode MatMPIDenseScatter(Mat A, Mat B, PetscInt Bbidx, Mat C, Mat *outworkB)
{
  ...
  PetscCall(MatMPIDenseScatter_Private(baij->Mvctx, baij->B->cmap->n, bs, contents->workB, contents->workB1, contents->blda, contents->rtype, contents->stype, contents->swaits, contents->rwaits, B, Bbidx, C, outworkB));
  PetscFunctionReturn(PETSC_SUCCESS);
}
